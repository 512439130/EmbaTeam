package com.fala.emba.team.view.time;
...
public class CustomDatePicker {
    ...
    private int scrollUnits = SCROLL_TYPE.YEAR.value + SCROLL_TYPE.MONTH.value + SCROLL_TYPE.DAY.value + SCROLL_TYPE.HOUR.value + SCROLL_TYPE.MINUTE.value;
    private ResultHandler handler;
    private Context context;
    private boolean canAccess = true;  //显示开关
    ...
    public CustomDatePicker(Context context, ResultHandler resultHandler, String title, String startDate, String endDate) {
        if (isValidDate(startDate, "yyyy-MM-dd HH:mm") && isValidDate(endDate, "yyyy-MM-dd HH:mm")) {
            LogManager.d(TAG, "时间格式：yyyy-MM-dd HH:mm");
            canAccess = true;
            ...
            initDialog();
            initView();
            initListener("yyyy-MM-dd HH:mm");
        } else if (checkTime(startDate) && checkTime(endDate)) {
            LogManager.d(TAG, "时间格式：HH:mm");
            canAccess = true;
            this.context = context;
            this.handler = resultHandler;
            selectedCalender = Calendar.getInstance();
    ...
    private void initDialog() {
        if (datePickerDialog == null) {
            ...
            datePickerDialog.setCancelable(true);  //点击屏幕外消失
    ...
    private void initView() {
        ...
        tvdialogTitle = datePickerDialog.findViewById(R.id.tv_title);
        tvdialogTitle.setText(dialogTitle);  // 设置标题文字

    }

    private void initListener(final String timeType) {
...
//        day_pv.postDelayed(new Runnable() {  // 刷新
//            @Override
//            public void run() {
//                hourChange();
//            }
//        }, 100);
    ...
    private int disScrollUnit(SCROLL_TYPE... scroll_types) {
        if (scroll_types == null || scroll_types.length == 0) {
            scrollUnits = SCROLL_TYPE.YEAR.value + SCROLL_TYPE.MONTH.value + SCROLL_TYPE.DAY.value + SCROLL_TYPE.HOUR.value + SCROLL_TYPE.MINUTE.value;
        } else {
...
//            }
     ...
     * 只显示 小时分钟
     */
    public void showSpecificYMD(boolean show) {
        if (canAccess) {
            if (show) {
                disScrollUnit();
    ...
    public void setSelectedTime(String time) {
        if (canAccess) {
            if (isValidDate(time, "yyyy-MM-dd HH:mm")) {  //yyyy-MM-dd HH:mm
                ...
                executeScroll();
            } else if (checkTime(time)) {  //HH:mm格式
